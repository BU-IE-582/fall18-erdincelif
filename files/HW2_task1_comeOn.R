...
## ComeOn
data_1 =odd_details[bookmaker=='ComeOn']
# data_x =odd_details[bookmaker=='ComeOn'& betType=='dc']
...
plot(pca, main = 'ComeOn odds - PCA analysis')
...
biplot(pca, main="ComeOn - PCA - Biplot")
plot(pca$scores[,1],pca$scores[,2],col=merged_all_1$IsOver+1,pch=".",cex=5, main = "ComeOn - PCA Analysis - Over/Under 2.5")
legend(-14, 12, legend=c("UNDER 2.5", "OVER 2.5"),
...
plot(mds.values_m[,1],mds.values_m[,2],xlab='', ylab='',col=1, pch='.',cex=5, main = "ComeOn - MDS plot using Manhattan distance")
...
mds.data <- data.frame(X=mds.values[,1],
                       Y=mds.values[,2])
mds.data
plot(mds.values[,1],mds.values[,2],pch='.',cex=5, main="ComeOn - MDS plot using Euclidean distance",xlab='', ylab='',col=1)
